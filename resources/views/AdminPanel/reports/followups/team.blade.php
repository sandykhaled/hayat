                    ...
                    <div class="tab-pane {{$activeChartTab}}" id="chart" aria-labelledby="chart-tab" role="tabpanel">
                        <div class="d-flex justify-content-center mb-3 text-center">
                            <div class="me-2">
                                ...
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(teamFollowupsStats($leader,$month,$year)['Mail'])}}</span>
                                </h3>
                            </div>
                            <div class="me-2">
                                <p class="card-text mb-50">{{trans('common.PhoneCall')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(teamFollowupsStats($leader,$month,$year)['Call'])}}</span>
                                </h3>
                            </div>
                            <div class="me-2">
                                <p class="card-text mb-50">{{trans('common.InVisit')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(teamFollowupsStats($leader,$month,$year)['InVisit'])}}</span>
                                </h3>
                            </div>
                            <div class="me-2">
                                <p class="card-text mb-50">{{trans('common.OutVisit')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(teamFollowupsStats($leader,$month,$year)['OutVisit'])}}</span>
                                </h3>
                            </div>
                            <div>
                                <p class="card-text mb-50">{{trans('common.UnitVisit')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(teamFollowupsStats($leader,$month,$year)['UnitVisit'])}}</span>
                                </h3>
                            </div>
                        </div>
                                ...
                                @foreach(monthsForThisYear($year) as $singleMonth)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            {{number_format(teamFollowupsStats($leader,$singleMonth,$year)['Mail'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(teamFollowupsStats($leader,$singleMonth,$year)['Call'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(teamFollowupsStats($leader,$singleMonth,$year)['InVisit'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(teamFollowupsStats($leader,$singleMonth,$year)['OutVisit'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(teamFollowupsStats($leader,$singleMonth,$year)['UnitVisit'])}}
                                        </td>
                                        <td class="text-center">
                                            <a href="{{route('admin.teamFollowUpsReport',['month'=>$singleMonth, 'year'=>$year, 'leader'=>$leader])}}" class="btn btn-icon btn-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="{{trans('common.view')}}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body pb-5 px-sm-5 pt-50">
                    ...
                    <div class="row justify-content-center">
                        ...
                        <div class="col-12 col-md-4">
                            <label class="form-label" for="leader">{{trans('common.agent')}}</label>
                            {{Form::select('leader',agentsListForSearch(),$leader,['id'=>'leader', 'class'=>'selectpicker','data-live-search'=>'true'])}}
                        </div>
                ...
                series: [
                    {
                        ...
                        data: [{{teamMonthFollowupsStats($leader,$month, $year)["Mail"]}}]
                    },
                    {
                        name: '{{trans("common.PhoneCall")}}',
                        data: [{{teamMonthFollowupsStats($leader,$month, $year)["Call"]}}]
                    },
                    {
                        name: '{{trans("common.InVisit")}}',
                        data: [{{teamMonthFollowupsStats($leader,$month, $year)["InVisit"]}}]
                    },
                    {
                        name: '{{trans("common.OutVisit")}}',
                        data: [{{teamMonthFollowupsStats($leader,$month, $year)["OutVisit"]}}]
                    },
                    {
                        name: '{{trans("common.UnitVisit")}}',
                        data: [{{teamMonthFollowupsStats($leader,$month, $year)["UnitVisit"]}}]
                    }
                ]
            };
