                    ...
                    <div class="tab-pane {{$activeChartTab}}" id="chart" aria-labelledby="chart-tab" role="tabpanel">
                        <div class="d-flex justify-content-center mb-3 text-center">
                            <div class="me-2">
                                ...
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(branchFollowupsStats($branch,$month,$year)['Mail'])}}</span>
                                </h3>
                            </div>
                            <div class="me-2">
                                <p class="card-text mb-50">{{trans('common.PhoneCall')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(branchFollowupsStats($branch,$month,$year)['Call'])}}</span>
                                </h3>
                            </div>
                            <div class="me-2">
                                <p class="card-text mb-50">{{trans('common.InVisit')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(branchFollowupsStats($branch,$month,$year)['InVisit'])}}</span>
                                </h3>
                            </div>
                            <div class="me-2">
                                <p class="card-text mb-50">{{trans('common.OutVisit')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(branchFollowupsStats($branch,$month,$year)['OutVisit'])}}</span>
                                </h3>
                            </div>
                            <div>
                                <p class="card-text mb-50">{{trans('common.UnitVisit')}}</p>
                                <h3 class="fw-bolder">
                                    <span class="text-info">{{number_format(branchFollowupsStats($branch,$month,$year)['UnitVisit'])}}</span>
                                </h3>
                            </div>
                        </div>
                                ...
                                @foreach(monthsForThisYear($year) as $singleMonth)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            {{number_format(branchFollowupsStats($branch,$singleMonth,$year)['Mail'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(branchFollowupsStats($branch,$singleMonth,$year)['Call'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(branchFollowupsStats($branch,$singleMonth,$year)['InVisit'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(branchFollowupsStats($branch,$singleMonth,$year)['OutVisit'])}}
                                        </td>
                                        <td class="text-center">
                                            {{number_format(branchFollowupsStats($branch,$singleMonth,$year)['UnitVisit'])}}
                                        </td>
                                        <td class="text-center">
                                            <a href="{{route('admin.branchFollowUpsReport',['month'=>$singleMonth, 'year'=>$year, 'branch'=>$branch])}}" class="btn btn-icon btn-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="{{trans('common.view')}}">
                    ...
                    <div class="row justify-content-center">
                        ...
                        @if(userCan('followups_view'))
                            <div class="col-12 col-md-4">
                                ...
                                {{Form::select('branch',['all'=>'الجميع'] + branchesList(),$branch,['id'=>'branch', 'class'=>'selectpicker','data-live-search'=>'true'])}}
                            </div>
                        @endif
                ...
                series: [
                    {
                        ...
                        data: [{{branchMonthFollowupsStats($branch,$month, $year)["Mail"]}}]
                    },
                    {
                        name: '{{trans("common.PhoneCall")}}',
                        data: [{{branchMonthFollowupsStats($branch,$month, $year)["Call"]}}]
                    },
                    {
                        name: '{{trans("common.InVisit")}}',
                        data: [{{branchMonthFollowupsStats($branch,$month, $year)["InVisit"]}}]
                    },
                    {
                        name: '{{trans("common.OutVisit")}}',
                        data: [{{branchMonthFollowupsStats($branch,$month, $year)["OutVisit"]}}]
                    },
                    {
                        name: '{{trans("common.UnitVisit")}}',
                        data: [{{branchMonthFollowupsStats($branch,$month, $year)["UnitVisit"]}}]
                    }
                ]
            };
